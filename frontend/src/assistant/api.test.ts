...
describe('condominium assistant API', () => {
  ...
  it('keeps request context on conversation and sends question separately', async () => {
    http.post.mockResolvedValueOnce({ data: { id: 'chat-1', requestId: 'request-1' } })
    ...
    expect(http.post).toHaveBeenNthCalledWith(1, '/condominiums/condo-1/assistant/conversations',
      { requestId: 'request-1', title: 'Consulta sobre solicitação' })
    expect(http.post).toHaveBeenNthCalledWith(2, '/condominiums/condo-1/assistant/conversations/chat-1/messages',
      { question: 'Qual regra se aplica?' })
  })

  it('supports paged history, reopening and deletion scoped by condominium', async () => {
    http.get.mockResolvedValueOnce({ data: { items: [], hasMore: false, total: 0 } })
      .mockResolvedValueOnce({ data: { conversation: { id: 'chat-1' }, messages: [] } })
    ...
    expect(http.get).toHaveBeenNthCalledWith(1, '/condominiums/condo-1/assistant/conversations',
      { params: { page: 2, pageSize: 20, search: 'piscina' } })
    expect(http.get).toHaveBeenNthCalledWith(2, '/condominiums/condo-1/assistant/conversations/chat-1')
    expect(http.delete).toHaveBeenCalledWith('/condominiums/condo-1/assistant/conversations/chat-1')
  })

  it('creates a conversation only together with the first question', async () => {
    http.post.mockResolvedValue({ data: { conversation: { id: 'chat-1' }, answer: 'ok', sources: [] } })
