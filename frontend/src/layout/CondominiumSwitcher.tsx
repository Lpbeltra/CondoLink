...
export function CondominiumSwitcher() {
  const { condominiums, currentCondominium, selectCondominium } =
    useCondominium();
    ...
    <>
      <ButtonBase
        ...
        sx={{
          ...
          "&:hover": {
            bgcolor: hasMultiple ? "background.default" : "background.paper",
          },
        }}
      >
        <ApartmentRoundedIcon color="primary" sx={{ mr: 1, flexShrink: 0 }} />
        <Box minWidth={0} textAlign="left" flex={1}>
          <Typography fontWeight={750} fontSize=".85rem" noWrap>
      ...
      >
        ...
        {condominiums.map((item) => {
          const selected =
            item.condominium.id === currentCondominium.condominium.id;
          return (
            <MenuItem
              key={item.membershipId}
              selected={selected}
              onClick={() => closeAndSelect(item.condominium.id)}
              sx={{ minHeight: 52, mx: 1, borderRadius: 2 }}
            >
              <ListItemText
                primary={item.condominium.name}
