...
const labels: Record<string, string> = { Assistant: 'Assistente', Documents: 'Documentos', Providers: 'Prestadores', ManagementCompanyRequests: 'Solicitações para Administradora' }

export function CondominiumModulesCard({ condominiumId }: { condominiumId: string }) {
  const [items, setItems] = useState<ModuleState[]>([])
  const [error, setError] = useState('')
  const [saving, setSaving] = useState<string | null>(null)
  useEffect(() => {
    setItems([]); setError('')
    void api.get<ModuleState[]>(`/overwatch/condominiums/${condominiumId}/modules`)
      .then(x => setItems(x.data.filter(item => item.module !== 'EmployeeManagement')))
  ...
  const save = async (next: ModuleState, previous: ModuleState[]) => {
    ...
    catch { setItems(previous); setError('Não foi possível salvar o módulo.') }
