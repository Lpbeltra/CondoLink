...
export function ManagementCompanyRequestTimeline({ request, administrator = false }: { request: RequestDetail; administrator?: boolean }) {
  ...
  return <Stack component="ol" sx={{ listStyle: "none", p: 0, m: 0 }}>{history.map((item, index) => {
    const cancelledBy = request.cancellationOrigin === "SubManager" ? "SubManager" : request.cancellationOrigin === "Manager" ? "Manager" : undefined;
    const title = item.eventType === "Cancelled" ? (request.cancellationOrigin === "ManagementCompany" ? "Cancelado pela Administradora" : cancelledBy ? `Cancelado pelo ${cancelledBy === "SubManager" ? "Subsíndico" : "Síndico"}` : "Cancelado") : item.reason || (item.previousStatus === null ? "Solicitação criada" : `Status alterado para ${administrator ? administratorRequestStatusLabel(item.newStatus, request.type) : statusLabel(item.newStatus, request.type)}`);
