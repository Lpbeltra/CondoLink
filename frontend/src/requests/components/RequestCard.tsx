...
export function RequestCard({ request }: { request: RequestListItem }) {
  ...
  return <Card elevation={0} sx={{ boxShadow: 'none', transition: 'border-color 150ms ease, transform 150ms ease', '&:hover': { borderColor: 'primary.light', transform: 'translateY(-1px)' } }}>
    <CardActionArea onClick={() => navigate(`/requests/${request.id}`)} sx={{ minHeight: 148 }}>
      <CardContent sx={{ p: { xs: 2.25, sm: 2.75 } }}>
        <Box display="flex" gap={1.5} alignItems="flex-start"><Box flex={1} minWidth={0}><Typography color="primary.main" fontSize=".76rem" fontWeight={800}>Atendimento #{formatRequestProtocol(request.id, request.protocol)}</Typography><Typography color="text.secondary" fontSize=".8rem" fontWeight={700} mt={.25}>{request.category.name}</Typography><Typography variant="h3" mt={.5}>{request.title}</Typography></Box><ChevronRightRoundedIcon color="action" /></Box>
