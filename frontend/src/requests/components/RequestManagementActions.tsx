...
export function RequestManagementActions({ requestId, status, priority, agendaReminder, onUpdated }: Props) {
  ...
  const generateSuggestion = async () => {
    if (!canSuggest || isSuggesting || reason.length > 3000 || !nextStatus) return
  ...
  const counterColor = reason.length >= 2700 ? (reason.length > 3000 ? 'error.main' : 'warning.main') : 'text.secondary'
  ...
  return <Box component="section" sx={{ mb: 2.5 }}>
    ...
    <Box mt={2} sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' }, gap: 1, '& .MuiButton-root': { width: '100%', minWidth: 0, whiteSpace: 'normal', justifyContent: 'flex-start', textAlign: 'left' }, '& > .MuiButton-root:first-of-type': { order: { xs: 1, sm: 0 }, gridRow: { sm: updateAllowed ? 2 : 1, xs: 'auto' } }, '& > .MuiButton-root:last-of-type': { order: { xs: 10, sm: 0 }, gridColumn: '1 / -1', gridRow: { sm: updateAllowed ? 4 : 3, xs: 'auto' } } }}>
    ...
    <Dialog open={statusOpen} onClose={() => { if (!isSaving && !isSuggesting) closeStatus() }} fullWidth maxWidth="sm">
      ...
      <DialogContent><Stack spacing={2} mt={1}>
        ...
        {suggestion && <Stack spacing={1.5}>
          ...
          <Box><Typography variant="subtitle2">Sugestão da IA</Typography><TextField fullWidth multiline minRows={3} value={suggestion} onChange={event => setSuggestion(event.target.value)} inputProps={{ maxLength: 3000 }} helperText={`${suggestion.length} / 3000`} /></Box>
