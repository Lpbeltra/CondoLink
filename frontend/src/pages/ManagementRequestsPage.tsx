...
export function ManagementRequestsPage() {
  ...
  return <PageContainer maxWidth={1440} sx={{ overflowX: 'hidden' }}>
    ...
    <Box display="grid" gridTemplateColumns={{ xs: 'minmax(0, 1fr)', sm: 'repeat(2, minmax(0, 1fr))', lg: 'repeat(4, minmax(0, 1fr))', xl: 'repeat(6, minmax(0, 1fr))' }} gap={1.5} my={3} alignItems="center">
      ...
      {!usesConsolidatedManagementScope && <FormControl size="small" sx={{ minWidth: 0 }}><InputLabel>Categoria</InputLabel><Select label="Categoria" value={applicableCategoryId} onChange={event=>setCategory(event.target.value)}><MenuItem value="">Todas</MenuItem>{categories.map(category=><MenuItem key={category.id} value={category.id}>{category.name}</MenuItem>)}</Select></FormControl>}
      <FormControl size="small" sx={{ minWidth: 0 }}><InputLabel>Status</InputLabel><Select label="Status" value={status} onChange={(event) => setFilter('status',event.target.value)}><MenuItem value="">Todos</MenuItem>{Object.entries(statusPresentation).map(([value, item]) => <MenuItem key={value} value={value}>{item.label}</MenuItem>)}</Select></FormControl>
      ...
      <FormControl size="small" sx={{ minWidth: 0 }}><InputLabel>Ordem</InputLabel><Select label="Ordem" value={direction} onChange={event=>setFilter('direction',event.target.value)}><MenuItem value="asc">Crescente</MenuItem><MenuItem value="desc">Decrescente</MenuItem></Select></FormControl>
