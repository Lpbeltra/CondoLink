...
import { Accordion, AccordionDetails, AccordionSummary, Alert, Box, Button, Card, CardActionArea, CardContent, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, Grid, InputAdornment, InputLabel, MenuItem, Select, Skeleton, Stack, TextField, Typography, useMediaQuery, useTheme } from '@mui/material'
  ...
  return <PageContainer maxWidth={1440} sx={{ overflowX: 'hidden' }}>
    ...
    {isMobile ? <>
      <Stack direction="row" gap={1} my={2}>
        <TextField fullWidth size="small" label="Buscar atendimentos" value={search} onChange={(event) => setFilter('search',event.target.value)} placeholder="Título, morador, categoria ou unidade" InputProps={{ startAdornment: <InputAdornment position="start"><SearchRoundedIcon /></InputAdornment> }} />
        <Button variant="outlined" startIcon={<FilterAltRoundedIcon />} onClick={() => setFiltersOpen(true)}>Filtrar</Button>
      </Stack>
      {requests}
      <Accordion elevation={0} sx={{ mt: 2 }} data-testid="management-request-summary">
        <AccordionSummary expandIcon={<ExpandMoreRoundedIcon />}><Typography fontWeight={700}>Resumo</Typography></AccordionSummary>
      ...
      <Dialog open={filtersOpen} onClose={() => setFiltersOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent><Box pt={1}>{detailedFilters}</Box></DialogContent>
        <DialogActions>
          <Button startIcon={<FilterAltOffRoundedIcon />} onClick={clearFilters} disabled={!status && !priority && !applicableCategoryId}>Limpar</Button>
          <Button variant="contained" onClick={() => setFiltersOpen(false)}>Concluir</Button>
        </DialogActions>
      </Dialog>
    </> : <>
      <Box mt={3}>{summary}</Box>
      <Box display="grid" gridTemplateColumns={{ sm: 'repeat(2, minmax(0, 1fr))', lg: 'repeat(4, minmax(0, 1fr))', xl: 'repeat(6, minmax(0, 1fr))' }} gap={1.5} my={3} alignItems="center">
        ...
        {detailedFilters}
        <Button sx={{ justifySelf: 'start' }} startIcon={<FilterAltOffRoundedIcon />} onClick={clearFilters} disabled={!status && !priority && !search && !applicableCategoryId}>Limpar filtros</Button>
      </Box>
      {requests}
    </>}
  </PageContainer>
