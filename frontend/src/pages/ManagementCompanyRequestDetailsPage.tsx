...
export function ManagementCompanyRequestDetailsPage() {
  ...
  const load = useCallback(async (showLoading = false) => { if (!id) return; if (showLoading) setLoading(true); try { setData(await getRequest(id)); setError(""); } catch { setError("Solicitação não encontrada ou fora do seu escopo."); } finally { if (showLoading) setLoading(false); } }, [id]);
  useEffect(() => { void load(true); }, [load]);
  useManagementCompanyRequestRealtime({ enabled: Boolean(id), onMessage: () => {}, onUpdated: () => void load(false) });
  const reply = async () => { if (!id || !text.trim() || sending) return; setSending(true); try { await interact(id, text.trim(), files); setText(""); setFiles([]); await load(false); } catch { setError("Não foi possível enviar. A solicitação pode ter sido atualizada; os dados foram recarregados."); await load(false); } finally { setSending(false); } };
  ...
  return <PageContainer maxWidth={1200}><TransientFeedback message={feedback} severity="success" onClose={() => setFeedback("")} /><Stack spacing={2}>
    {error && <Alert severity="error">{error}</Alert>}<Box display="flex" justifyContent="space-between" gap={2} flexWrap="wrap"><Box><Typography variant="h1">{data.friendlyIdentifier}</Typography><Typography>{typeLabel[data.type]} · {data.condominiumName} · {data.managementCompanyName}</Typography></Box><Chip color={data.status === "Completed" ? "success" : data.status === "Cancelled" ? "error" : data.status === "Submitted" ? "primary" : "default"} label={statusLabel(data.status, data.type)} /></Box>
    <Grid container spacing={3}><Grid size={{ xs: 12, lg: 8 }}><Stack spacing={2}><RequestInformationCards request={data} />
      {!terminal && <ManagementCompanyRequestActionCard><Button variant="outlined" onClick={() => nav(`/management/administrator/${data.id}/edit`)}>Editar solicitação</Button><Button color="error" variant="outlined" onClick={() => setCancelOpen(true)}>Cancelar solicitação</Button></ManagementCompanyRequestActionCard>}
