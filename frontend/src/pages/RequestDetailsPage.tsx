...
import { RequestHistory } from '../requests/components/RequestTimeline'
...
type DetailTab = 'history' | 'data' | 'provider' | 'attachments' | 'notes'

function normalizedDetailTab(search: string): DetailTab {
  const value = new URLSearchParams(search).get('tab')
  if (value === 'conversation' || value === 'timeline') return 'history'
  return value === 'history' || value === 'provider' || value === 'attachments' || value === 'notes' ? value : 'data'
}
...
function ContextPanel({ details, unit, managementMode, onAssistant }: { details: RequestDetails; unit: string; managementMode: boolean; onAssistant?: () => void }) {
  return <Paper component="aside" elevation={0} variant="outlined" sx={{ p: 2 }}>
...
export function RequestDetailsPage({ managementCondominiumId, managementMode = false }: RequestDetailsPageProps = {}) {
  ...
  const [details, setDetails] = useState<RequestDetails | null>(null); const [messages, setMessages] = useState<RequestMessage[]>([]); const [isLoading, setIsLoading] = useState(true); const [error, setError] = useState(''); const [actionFeedback, setActionFeedback] = useState(''); const [tab, setTab] = useState<DetailTab>(() => normalizedDetailTab(location.search))
  ...
  return <PageContainer maxWidth={1440}>
    <Box display="grid" gridTemplateColumns={{ xs: '1fr', lg: 'minmax(0, 1fr) 250px' }} gap={{ xs: 2, lg: 3 }} alignItems="start"><Box minWidth={0}>
      ...
      <Tabs value={tab} onChange={(_, value: DetailTab) => setTab(value)} variant="scrollable" allowScrollButtonsMobile sx={{ borderBottom: 1, borderColor: 'divider', mb: 2 }} aria-label="Áreas do atendimento"><Tab value="data" label="Dados" /><Tab value="history" label="Histórico" /><Tab value="provider" label="Prestador" /><Tab value="attachments" label="Anexos" /><Tab value="notes" label={tabLabel('Notas', details.internalNotes?.length ?? 0)} /></Tabs>
      <Box sx={show('history')}><RequestHistory history={details.statusHistory} messages={messages} serviceProviderHistory={managementMode ? (details.serviceProviderHistory ?? []) : []} providerPaymentRequests={managementMode ? (details.providerPaymentRequests ?? []) : []} />{!managementMode && !residentReadOnly && !residentClosurePending && !details.residentReplyRequirement && <ResidentUpdateDialog requestId={details.id} onSent={async message => { setMessages(current => [...current, message]); await load(true) }} />}{!managementMode && details.residentReplyRequirement && <ResidentReplyPanel requestId={details.id} requirement={details.residentReplyRequirement} onSent={load} />}</Box>
    ...
    </Box><Box sx={{ position: { lg: 'sticky' }, top: { lg: 84 }, alignSelf: 'start' }}>{canViewInternal && <RequestManagementActions requestId={details.id} status={details.status} priority={details.priority} agendaReminder={details.agendaReminder} onUpdated={load} />}<ContextPanel details={details} unit={unit} managementMode={managementMode} onAssistant={managementMode ? () => navigate(`/management/assistant?requestId=${details.id}`) : undefined} />{!managementMode && details.residentClosureProposal && <ResidentClosurePanel requestId={details.id} proposal={details.residentClosureProposal} onUpdated={async feedback => { if (feedback) setActionFeedback(feedback); await load() }} />}</Box></Box>
  </PageContainer>
}
