...
function ContextPanel({ details, unit, managementMode }: { details: RequestDetails; unit: string | null; managementMode: boolean }) {
...
export function RequestDetailsPage({ managementCondominiumId, managementMode = false }: RequestDetailsPageProps = {}) {
  ...
  const canViewInternal = details.canManageInternalNotes ?? canViewInternalRequestDetails(managementMode, isManager, details.condominiumId, expectedCondominiumId)
  const show = (name: DetailTab) => ({ display: tab === name ? 'block' : 'none' })
  const tabLabel = (label: string, count?: number) => count ? `${label} (${count})` : label

  return <PageContainer maxWidth={1440}>
    <PageHeader eyebrow={`Atendimento · #${formatRequestProtocol(details.id, details.protocol)}`} title={details.title} description={`${details.author.fullName}${unit ? ` · ${unit}` : ''} · ${details.category.name}`} actions={<><Button startIcon={<ArrowBackRoundedIcon />} color="inherit" onClick={() => navigate(returnPath)}>Voltar</Button>{managementMode && <Button variant="outlined" onClick={() => navigate(`/management/assistant?requestId=${details.id}`)}>Consultar assistente</Button>}</>} />
    ...
    {(location.state as { created?: boolean } | null)?.created && <Alert severity="success" sx={{ mb: 1.5 }}>Solicitação aberta com sucesso.</Alert>}
    {actionFeedback && <Alert severity="success" sx={{ mb: 1.5 }}>{actionFeedback}</Alert>}
    {residentReadOnly && <Alert severity="info" sx={{ mb: 1.5 }}>Esta solicitação está encerrada e disponível somente para consulta.</Alert>}
    {managementMode && details.status === 'WaitingForResidentClosure' && <Alert severity="warning" sx={{ mb: 1.5 }}>A administração concluiu este atendimento e aguarda a confirmação do morador.</Alert>}
    {managementMode && details.hasUnreadResidentReply && <Alert severity="warning" sx={{ mb: 1.5 }}>Morador respondeu — requer andamento.</Alert>}
    ...
    <Box display="grid" gridTemplateColumns={{ xs: '1fr', lg: 'minmax(0, 1fr) 250px' }} gap={{ xs: 2, lg: 3 }}>
      <Box minWidth={0}>
        <Tabs value={tab} onChange={(_, value: DetailTab) => setTab(value)} variant="scrollable" allowScrollButtonsMobile sx={{ borderBottom: 1, borderColor: 'divider', mb: 2 }} aria-label="Áreas do atendimento">
          <Tab value="conversation" label="Conversa" /><Tab value="timeline" label="Timeline" /><Tab value="data" label="Dados" /><Tab value="provider" label="Prestador" /><Tab value="attachments" label="Anexos" /><Tab value="notes" label={tabLabel('Notas', details.internalNotes?.length ?? 0)} />
        </Tabs>
        <Box sx={show('conversation')}>
          <RequestConversation requestId={details.id} status={details.status} messages={messages} readOnly={residentReadOnly || residentClosurePending || (!managementMode && Boolean(details.residentReplyRequirement))} showComposer={false} onMessageCreated={message => setMessages(current => [...current, message])} />
          {!managementMode && !residentReadOnly && !residentClosurePending && !details.residentReplyRequirement && <ResidentUpdateDialog requestId={details.id} onSent={async message => { setMessages(current => [...current, message]); await load(true) }} />}
        ...
        <Box sx={show('data')}>
          {managementMode && details.residentSummary && <ResidentSummaryCard resident={details.residentSummary} />}
          <Box component="section" sx={{ py: 1 }}><Typography variant="h2" mb={1}>Dados do atendimento</Typography><Typography color="text.secondary" fontSize=".85rem">{details.author.fullName}{unit ? ` · ${unit}` : ''}</Typography><Divider sx={{ my: 2 }} /><Typography variant="h3" mb={.75}>Resumo</Typography><Typography sx={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{canViewInternal ? details.mainDescription?.trim() || details.aiAnalysis?.description?.trim() || details.description : details.description}</Typography>{canViewInternal && details.aiAnalysis?.suggestedCategory?.trim() && <Typography color="text.secondary" fontSize=".82rem" mt={1}>Sugestão da IA: {details.aiAnalysis.suggestedCategory}</Typography>}<OriginalReportAccordion key={details.id} requestId={details.id} report={details.originalReport} messages={messages} authorId={details.author.id} portalDescription={details.description} requestCreatedAt={details.createdAt} /></Box>
        </Box>
        <Box sx={show('provider')}>{managementMode && canViewInternal && <RequestServiceProviderCard requestId={details.id} status={details.status} current={details.serviceProvider} history={details.serviceProviderHistory ?? []} paymentRequests={details.providerPaymentRequests ?? []} requestTitle={details.title} condominium={currentCondominium?.condominium.name} unit={unit} onUpdated={() => load(true)} />}</Box>
        <Box sx={show('attachments')}><RequestAttachments requestId={details.id} readOnly={residentReadOnly || residentClosurePending || Boolean(!managementMode && details.residentReplyRequirement)} /></Box>
        <Box sx={show('notes')}>{canViewInternal && details.canManageInternalNotes && <RequestInternalNotes requestId={details.id} onChanged={() => load(true)} />}</Box>
      </Box>
      <Box>
        <ContextPanel details={details} unit={unit} managementMode={managementMode} />
