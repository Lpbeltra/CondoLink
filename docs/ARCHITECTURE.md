...
SubManager continua sendo uma role contextual de um único vínculo de condomínio, não um RBAC genérico. A tabela `sub_manager_module_permissions` guarda uma linha tipada por módulo, com concessão/revogação e usuário auditor. `Requests` permanece apenas como valor histórico persistido; Atendimento usa `Attendance`. Os seis módulos configuráveis são `Attendance`, `ManagementCompany`, `Agenda`, `Assistant`, `Documents` e `Management`. Manager mantém o comportamento integral atual. Vínculos SubManager sem linhas usam compatibilidade integral; novos vínculos são inicializados com os seis módulos configuráveis habilitados, sem renumerar valores históricos.
