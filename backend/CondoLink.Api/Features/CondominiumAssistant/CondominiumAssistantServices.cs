    ...
    AssistantOperationalTools? operationalTools = null)
{
        ...
        CondominiumAssistantChannel channel = CondominiumAssistantChannel.Portal)
    {
        ...
        try
        {
            ...
            var groundingApplied = !streamed.HasSuccessfulOperationalQuery;
            var answerBeforeGrounding = answer;
            answer = EnforceGrounding(answer, prepared.Evidence, streamed.HasSuccessfulOperationalQuery);
            logger.LogInformation("Assistant grounding decision. CondominiumId: {CondominiumId}; ConversationId: {ConversationId}; OperationalReferences: {OperationalReferences}; DocumentaryEvidence: {DocumentaryEvidence}; Applied: {Applied}; AnswerChanged: {AnswerChanged}; AnswerCharacters: {AnswerCharacters}.",
        ...
        string? requestHint, IReadOnlyList<RankedChunk> candidates, string reason, CancellationToken ct)
    {
        ...
        var settings = aiOptions.Value;
        if (!settings.Enabled || string.IsNullOrWhiteSpace(settings.ApiKey))
            return new(HybridRanking(candidates), false, false, false, false,
                0, 0, 0, null, 0);
        var started = System.Diagnostics.Stopwatch.StartNew();
        var payloadBytes = 0;
        using var retryTracker = OpenAiRetryTracker.Begin();
        try
        {
            ...
            started.Stop();
            return new(candidates.Select(x => x with
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            ...
            return new(HybridRanking(candidates), true, false, false, true,
    ...
    {
        if (hasSuccessfulOperationalQuery) return answer;
