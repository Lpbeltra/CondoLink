...
public static class ManagementCompanyRequestEndpoints
{
    public static IEndpointRouteBuilder MapManagementCompanyRequests(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/{id:guid}/attachments",UploadAttachments).DisableAntiforgery()
    ...
    {
        page=Math.Max(1,page);pageSize=Math.Clamp(pageSize,1,50);var userId=await access.RequireUserIdAsync(user,ct);
        ...
        if(from.HasValue&&to.HasValue&&from>to)throw new ValidationAppException("A data inicial não pode ser posterior à data final.");var query=db.ManagementCompanyRequests.AsNoTracking().Where(r=>scoped.Contains(r.CondominiumId));if(condominiumId.HasValue)query=query.Where(r=>r.CondominiumId==condominiumId);if(type.HasValue)query=query.Where(r=>r.Type==type);if(status.HasValue)query=query.Where(r=>r.Status==status);if(from.HasValue){var start=DateTime.SpecifyKind(from.Value.ToDateTime(TimeOnly.MinValue),DateTimeKind.Utc);query=query.Where(r=>r.CreatedAt>=start);}if(to.HasValue){var end=DateTime.SpecifyKind(to.Value.AddDays(1).ToDateTime(TimeOnly.MinValue),DateTimeKind.Utc);query=query.Where(r=>r.CreatedAt<end);}
        ...
        var total=await query.CountAsync(ct);var items=await query.OrderByDescending(r=>r.Status==ManagementCompanyRequestStatus.WaitingManager).ThenByDescending(r=>r.UpdatedAt).Skip((page-1)*pageSize).Take(pageSize).Select(r=>new{r.Id,r.FriendlyIdentifier,r.CondominiumId,CondominiumName=db.Condominiums.Where(c=>c.Id==r.CondominiumId).Select(c=>c.Name).First(),r.ManagementCompanyId,ManagementCompanyName=db.ManagementCompanies.Where(c=>c.Id==r.ManagementCompanyId).Select(c=>c.Name).First(),r.Type,r.Status,Subject=r.Type==ManagementCompanyRequestType.Fine?db.ManagementCompanyFineRequests.Where(x=>x.RequestId==r.Id).Select(x=>x.Nature).First():r.Type==ManagementCompanyRequestType.Payment?db.ManagementCompanyPaymentRequests.Where(x=>x.RequestId==r.Id).Select(x=>x.Nature).First():db.ManagementCompanyGeneralQuestionRequests.Where(x=>x.RequestId==r.Id).Select(x=>x.Theme).First(),r.CreatedAt,r.UpdatedAt}).ToListAsync(ct);return Results.Ok(new{items,page,pageSize,total,hasMore=page*pageSize<total});
    }
    ...
    {
        await access.RequireManagementAsync(user,condominiumId,ct);var link=await db.CondominiumManagementCompanyLinks.AsNoTracking().SingleOrDefaultAsync(x=>x.CondominiumId==condominiumId&&x.IsActive,ct);if(link is null)return Results.Ok(new{condominiumId,managementCompany=(object?)null,categories=Array.Empty<object>(),units=Array.Empty<object>(),beneficiaries=Array.Empty<object>()});
    ...
    {
        var actor=await access.RequireForRequestAsync(user,id,ct);var request=await db.ManagementCompanyRequests.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==id,ct)??throw new NotFoundAppException("Solicitação não encontrada.");
        ...
        try
        {
            foreach(var file in files){var valid=AttachmentPolicy.Validate(file.FileName,file.Length,file.ContentType);if(valid.Error is not null)throw new ValidationAppException(valid.Error);var key=await storage.SaveManagementCompanyRequestAsync(id,file,valid.Extension!,ct);saved.Add(key);db.ManagementCompanyRequestAttachments.Add(new(id,actor.UserId,valid.Name!,key,valid.ContentType!,file.Length,messageId));}
            await db.SaveChangesAsync(ct);
        }
        catch{foreach(var key in saved)storage.Delete(key);throw;}
        return Results.Ok(await db.ManagementCompanyRequestAttachments.AsNoTracking().Where(x=>x.RequestId==id).Select(x=>new{x.Id,x.MessageId,x.OriginalFileName,x.ContentType,x.FileSize,x.CreatedAt}).ToListAsync(ct));
    }
    ...
    {await access.RequireForRequestAsync(user,id,ct);return Results.Ok(await db.ManagementCompanyRequestAttachments.AsNoTracking().Where(x=>x.RequestId==id).Select(x=>new{x.Id,x.MessageId,x.OriginalFileName,x.ContentType,x.FileSize,x.CreatedAt}).ToListAsync(ct));}
    ...
    private static async Task<object> ToDetail(Guid id,AppDbContext db,CancellationToken ct)
    {
        ...
        var history=await db.ManagementCompanyRequestHistories.AsNoTracking().Where(x=>x.RequestId==id).OrderBy(x=>x.CreatedAt).Select(x=>new{x.Id,x.EventType,x.PreviousStatus,x.NewStatus,x.ChangedByUserId,x.Reason,x.CreatedAt}).ToListAsync(ct);
        var attachments=await db.ManagementCompanyRequestAttachments.AsNoTracking().Where(x=>x.RequestId==id).OrderBy(x=>x.CreatedAt).Select(x=>new{x.Id,x.MessageId,x.OriginalFileName,x.ContentType,x.FileSize,x.CreatedAt}).ToListAsync(ct);
        return new{r.Id,r.FriendlyIdentifier,r.CondominiumId,condominiumName,r.ManagementCompanyId,managementCompanyName,r.CategoryId,r.CreatedByUserId,r.Type,r.Status,r.CreatedAt,r.UpdatedAt,r.AcknowledgedAt,r.AcknowledgedByUserId,r.CompletedAt,r.CompletedByUserId,r.CancelledAt,r.CancelledByUserId,r.CancellationReason,condominium=new{condominium.Name,condominium.Address,condominium.City,condominium.State,managers},fine,payment,question,messages,history,attachments};
    }
