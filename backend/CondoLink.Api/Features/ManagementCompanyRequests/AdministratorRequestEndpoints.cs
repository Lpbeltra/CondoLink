...
public static class AdministratorRequestEndpoints
{
    ...
    private static async Task<IResult> List(ClaimsPrincipal user, AppDbContext db, ManagementCompanyRequestAccessService access, CancellationToken ct, Guid? condominiumId = null, Guid? categoryId = null, ManagementCompanyRequestStatus? status = null, string? search = null, DateOnly? from = null, DateOnly? to = null, bool includeCompleted = false, bool includeCancelled = false, int page = 1, int pageSize = 20)
    {
        ...
        {
            var states = new List<ManagementCompanyRequestStatus>();
            if (includeCompleted) states.Add(ManagementCompanyRequestStatus.Completed);
            if (includeCancelled) states.Add(ManagementCompanyRequestStatus.Cancelled);
            if (states.Count == 1) query = query.Where(x => x.Status == states[0]);
            else if (states.Count > 1) query = query.Where(x => states.Contains(x.Status));
            else query = query.Where(x => x.Status != ManagementCompanyRequestStatus.Completed && x.Status != ManagementCompanyRequestStatus.Cancelled);
        }
