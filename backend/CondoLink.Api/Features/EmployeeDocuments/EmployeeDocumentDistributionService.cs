...
public sealed class EmployeeDocumentDistributionService(AppDbContext db, IOptions<WhatsAppOptions> whatsAppOptions)
{
    ...
    {
        var confirmed = await db.EmployeeDocuments.AsNoTracking().Where(x => x.BatchId == batchId && x.IdentificationStatus == EmployeeDocumentIdentificationStatus.Confirmed).Select(x => new { x.Id, x.EmployeeId }).ToArrayAsync(ct);
    ...
    {
        var batch = await db.EmployeeDocumentBatches.SingleAsync(x => x.Id == batchId && x.ManagementCompanyId == managementCompanyId, ct);
        ...
        var docs=await db.EmployeeDocuments.Where(x=>x.BatchId==batchId&&x.IdentificationStatus==EmployeeDocumentIdentificationStatus.Confirmed&&!queued.Contains(x.Id)).ToArrayAsync(ct); var employees=await db.Employees.AsNoTracking().Where(x=>docs.Select(d=>d.EmployeeId!.Value).Contains(x.Id)).ToDictionaryAsync(x=>x.Id,ct); var now=DateTime.UtcNow; var count=0;
        ...
        batch.StartDistributing(); try{await db.SaveChangesAsync(ct);}catch(DbUpdateException){return 0;} await TryCompleteBatchAsync(batchId,ct); return count;
    }
    ...
    public async Task<bool> RetryAsync(Guid batchId,Guid documentId,Guid actorUserId,Guid managementCompanyId,CancellationToken ct){var d=await db.EmployeeDocumentDeliveries.SingleOrDefaultAsync(x=>x.BatchId==batchId&&x.EmployeeDocumentId==documentId,ct);if(d is null)return false;var o=await db.WhatsAppOutboundMessages.SingleAsync(x=>x.Id==d.OutboundMessageId,ct);if(o.Status is not(WhatsAppOutboundStatus.Failed or WhatsAppOutboundStatus.PermanentlyFailed))return false;var doc=await db.EmployeeDocuments.SingleAsync(x=>x.Id==documentId&&x.BatchId==batchId,ct);var e=await db.Employees.SingleAsync(x=>x.Id==d.EmployeeId);if(doc.EmployeeId!=e.Id||doc.CondominiumId!=e.CondominiumId||!await db.Condominiums.AnyAsync(c=>c.Id==e.CondominiumId&&c.ManagementCompanyId==managementCompanyId&&c.IsActive,ct))return false;var t=whatsAppOptions.Value.Templates.EmployeePayslipAvailable;var n=new WhatsAppOutboundMessage(null,null,actorUserId,e.CondominiumId,o.DestinationPhone,WhatsAppNotificationType.EmployeePayslipAvailable,WhatsAppSendMode.Template,$"employee-document:{documentId}:whatsapp:{d.AttemptCount+1}","Holerite disponível.",t.Name,t.Language,DateTime.UtcNow,employeeDocumentId:documentId);db.WhatsAppOutboundMessages.Add(n);d.Retry(n.Id,actorUserId,DateTime.UtcNow);(await db.EmployeeDocumentBatches.SingleAsync(x=>x.Id==batchId)).StartDistributing();await db.SaveChangesAsync(ct);return true;}
}
