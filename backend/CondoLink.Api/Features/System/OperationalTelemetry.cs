...
public sealed class ApiRequestMetrics
{
    ...
    // Realtime connections are intentionally long-lived; exclude them from HTTP latency health.
    internal static bool IsHeavy(string route) => route.Contains("setup/confirm",StringComparison.OrdinalIgnoreCase)||route.Contains("import",StringComparison.OrdinalIgnoreCase)||route.Contains("export",StringComparison.OrdinalIgnoreCase)||route.Contains("documents",StringComparison.OrdinalIgnoreCase)||route.Contains("management-company-requests/realtime",StringComparison.OrdinalIgnoreCase);
